...
# Seleção de datas (distribuídas no período)
...
if len(sel_times) == 0:
    ...
    # Exibe em linhas de até 4 colunas
    per_row = 4
    ...
    for r in range(rows):
        ...
        for j, col in enumerate(cols, start=0):
            ...
            # Substitui NaN por vmin para visual limpa
            arr = np.where(np.isfinite(arr), arr, vmin)
            png = ndvi_slice_png(arr, vmin=vmin, vmax=vmax)
            with col:
                st.image(png, use_column_width=True, caption=pd.to_datetime(str(t)).strftime("%Y-%m-%d"))
